...
public String GetSalaryInfoUpdate( String js_input) {
    ...
    try{
		...
		SQL = "select * from salarylist where id="+id+" and employee_email  = '" + employee_email  +"'";
		ResultSet rset = stmt.executeQuery(SQL);
		while (rset.next()) {
			existing = 1;
		}
		rset.close(); 
		if(existing==1){
			SQL="update salarylist set employee_name= '"+employee_name+"', class_name ='"+class_name+"',salary ='"+salary+"',";
			SQL=SQL+" travel_allowance = '"+travel_allowance+"', medical_allowance = '"+medical_allowance+"',overtime_allowance = '"+overtime_allowance+"',month = '"+month+"' ,current_year = '"+current_year+"' ";
			SQL=SQL+" where id="+id+" and employee_email = '"+employee_email+"'";
...
//delete
public String GetSalaryInfoDelete( String js_input) {
	...
	Statement stmt = null;
	int existing=0,id=0;
	String salaryForm="", SQL="",employee_email ="";
	...
	try{
		...
		rset.close(); 
		if(existing==1){
			SQL="delete from salarylist where id="+id+" and employee_email  = '"+employee_email +"'";
